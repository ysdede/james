...
if jh.python_version() < 3.7:
    print(
        jh.color(
            f'Jesse James requires Python version above 3.7. Yours is {jh.python_version()}',
            'red'
        )
    )
...
def pick(dnalogfile: str, sortcriteria: str, len1: int, len2: int) -> None:
    ...
    import jesse.helpers as jh
    from james.dnasorter import sortdnas, valideoutputfile
...
def random(start_date: str, finish_date: str, iterations: int, days: int) -> None:
    ...
    random walk backtest. Enter period "YYYY-MM-DD" "YYYY-MM-DD
                                iterations eg. 100
                                window width in days eg 180" TODO Calculate it!
    """
    os.chdir(os.getcwd())
    validate_cwd()
    validateconfig()
    from jesse.routes import router
    import jesse.helpers as jh
    from james.timemachine import run
    r = router.routes[0]  # Read first route from routes.py
...
def refine(start_date: str, finish_date: str) -> None:
    ...
    from jesse.routes import router
    import jesse.helpers as jh
    from james.launcher import run
    validateconfig()
    makedirs()
...
def makedirs():
    ...
    os.makedirs(f'./{jamesdir}', exist_ok=True)
    os.makedirs(f'./{jamesdir}/results', exist_ok=True)
    os.makedirs(f'./{jamesdir}/logs', exist_ok=True)
    os.makedirs(f'./{jamesdir}/dnafiles', exist_ok=True)
