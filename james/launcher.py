...
def run(_start_date, _finish_date):
    ...
    reportfilename = f'{jamesdir}/results/{filename}--{ts}.csv'
    logfilename = f'{jamesdir}/logs/{filename}--{ts}.log'
    f = open(logfilename, 'w')
    f.write(str(headerforfiles) + '\n')

    dnasmodule = importlib.import_module(f'{jamesdir}.dnafiles.{strategy}dnas')
    ...
    dnafilename = f'{jamesdir}/dnafiles/{strategy}dnas.py'
